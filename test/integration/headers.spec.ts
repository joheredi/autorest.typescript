...
describe("typescript", function() {
  describe("Swagger Header BAT", function() {
    describe("Basic Header Operations", function() {
      var testClient = new HeaderClient(clientOptions);
      it("should override existing headers (nodejs only)", async function() {
        const result = await testClient.header.paramExistingKey("overwrite");
        result._response.status.should.be.equal(200);
      ...
      it("should send and receive date type headers", async function() {
        ...
        response1.value!.should.deep.equal(new Date("2010-01-01"));

        const response2 = await testClient.header.responseDate("min");
        response2.value!.should.deep.equal(new Date("0001-01-01"));
      });
      ...
      it("should send and receive datetime type headers", async function() {
        ...
        response1.value!.should.deep.equal(new Date("2010-01-01T12:34:56Z"));

        const response2 = await testClient.header.responseDatetime("min");

        response2.value!.should.deep.equal(new Date("0001-01-01T00:00:00Z"));
      });
      ...
      it.skip("should send and receive datetimerfc1123 type headers", async function() {
        ...
        response1.value!.should.deep.equal(
        ...
        response2.value!.should.deep.equal(
          new Date("Mon, 01 Jan 0001 00:00:00 GMT").valueOf()
        );
      });
      ...
      it("should send and receive duration type headers", async function() {
        ...
        response.value!.should.equal("P123DT22H14M12.011S");
      });
