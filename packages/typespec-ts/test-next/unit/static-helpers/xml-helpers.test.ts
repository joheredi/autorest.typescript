...
describe("XML Helpers", () => {
  ...
  describe("parseXmlString", () => {
    ...
    it("should preserve string values that look like numbers", () => {
      // Regression test for https://github.com/Azure/autorest.typescript/issues/3775
      // The value "1.0" must not be coerced to the number 1 for string-typed properties
      const xml = "<StorageServiceProperties><Version>1.0</Version></StorageServiceProperties>";

      const result = parseXmlString(xml);

      // The value "1.0" must remain as string "1.0", not be coerced to number 1
      expect(result.StorageServiceProperties[0].Version).toBe("1.0");
    });
  });
      ...
      const properties: XmlPropertyDeserializeMetadata[] = [
        {
          ...
          xmlOptions: { name: "data" },
          type: "object",
          deserializer: (v) => ({ name: v.name, value: Number(v.value) })
        }
      ];
...
<Model>
  ...
  <age>42</age>
      ...
      const properties: XmlPropertyDeserializeMetadata[] = [
        ...
        { propertyName: "age", xmlOptions: { name: "age" }, type: "primitive", primitiveSubtype: "number" }
      ];
...
<ModelWithEncodedNamesSrc>
      ...
      const properties: XmlPropertyDeserializeMetadata[] = [
        {
          ...
          type: "object",
          deserializer: (val: any) => ({ name: val.name, age: Number(val.age) })
        },
        {
      ...
      const properties: XmlPropertyDeserializeMetadata[] = [
        {
          ...
          xmlOptions: { name: "data" },
          type: "object",
          deserializer: (val: any) => ({ id: Number(val.id), label: val.label })
        },
        {
          propertyName: "tags",
  ...
  describe("deserializeXmlObject", () => {
    it("should deserialize a pre-parsed XML object with XML property names", () => {
      ...
      // With parseTagValue: false the parser returns strings, not booleans/numbers
      const xmlObject = {
        Enabled: "true",
        Days: "7"
      };
    ...
    it("should deserialize nested objects using XML property names", () => {
      ...
      // Outer object as it would be parsed from XML (strings from parser)
    ...
    it("should deserialize unix timestamp dates", () => {
      // With parseTagValue: false, timestamp is returned as string from parser
      const xmlObject = {
        Created: "1690891200"
      };
  ...
  describe("Round-trip serialization/deserialization", () => {
    it("should round-trip a simple model", () => {
      ...
      const serializeProperties: XmlPropertyMetadata[] = [
        {
          propertyName: "name",
          xmlOptions: { name: "name" },
          type: "primitive"
      ...
      const deserializeProperties: XmlPropertyDeserializeMetadata[] = [
        {
          propertyName: "name",
          xmlOptions: { name: "name" },
          type: "primitive",
      ...
      const xml = serializeToXml(original, serializeProperties, "Model");
      const result = deserializeFromXml(xml, deserializeProperties, "Model");
