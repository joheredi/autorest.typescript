...
export function isLro(_program: Program, operation: Operation): boolean {
...
function emitType(
  ...
  switch (type.kind) {
    ...
    case "Union":
      ...
      const enumName = modelTypeProperty
        ? capitalize(modelTypeProperty.name) + "Type"
        : "MyEnum";
      return {
        name: enumName,
        snakeCaseName: camelToSnakeCase(enumName),
        description: modelTypeProperty
          ? `Type of ${modelTypeProperty.name}.`
          : "n/a",
        isPublic: false,
        type: "enum",
...
function emitServerParams(
  ...
  if (server.parameters) {
    ...
    for (const param of server.parameters.values()) {
      ...
      if (
        isApiVersion(program, serverParameter as any) &&
...
export function emitClients(program: Program): Record<string, any>[] {
...
export function createYamlEmitter(program: Program) {
  ...
  for (const namespace of getNamespaces(program)) {
    if (namespace === serviceNamespaceString) {
      codeModel["clients"] = emitClients(program);
    } else {
      codeModel["subnamespaceToClients"][namespace] = emitClients(program);
    }
  }
  codeModel["types"] = [
    ...typesMap.values(),
