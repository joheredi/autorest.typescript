...
export function createChatProtocol(endpointParam: string, credential: (KeyCredential | TokenCredential), options: ChatProtocolClientOptionalParams = {}): ChatProtocolContext {
    ...
    const { apiVersion: _, ...updatedOptions } = {...options,userAgentOptions: { userAgentPrefix },loggingOptions: { logger: options.loggingOptions?.logger ?? logger.info },credentials: { scopes: options.credentials?.scopes ?? [`${endpointUrl}/.default`],apiKeyHeaderName: options.credentials?.apiKeyHeaderName ?? "api-key", },};
