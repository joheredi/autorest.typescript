...
import { BudgetsContinueOptionalParams, BudgetsGetBudgetsOptionalParams, BudgetsCreateOrReplaceOptionalParams } from "../../api/budgets/options.js";
...
function _getBudgets(context: SAPWidgetServiceContext) {
    return {
            continue: (options?: BudgetsContinueOptionalParams) => $continue(context,options),getBudgets: (name: string,options?: BudgetsGetBudgetsOptionalParams) => getBudgets(context,name,options),createOrReplace: (name: string,resource: SAPUser,options?: BudgetsCreateOrReplaceOptionalParams) => createOrReplace(context,name,resource,options)
          }
}
