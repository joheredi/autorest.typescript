...
import { analyzeWidget, deleteWidget, updateWidget, createOrReplace, createWidget, getWidget, queryWidgetsPages, listWidgetsPages, sapListWidgets } from "../../api/sapWidgets/operations.js";
...
function _getSAPWidgets(context: SAPWidgetServiceContext) {
    return {
            analyzeWidget: (id: string,options?: SAPWidgetsAnalyzeWidgetOptionalParams) => analyzeWidget(context,id,options),deleteWidget: (id: string,options?: SAPWidgetsDeleteWidgetOptionalParams) => deleteWidget(context,id,options),updateWidget: (id: string,options?: SAPWidgetsUpdateWidgetOptionalParams) => updateWidget(context,id,options),createOrReplace: (name: string,resource: SAPUser,options?: SAPWidgetsCreateOrReplaceOptionalParams) => createOrReplace(context,name,resource,options),createWidget: (weight: number,color: "red" | "blue",options?: SAPWidgetsCreateWidgetOptionalParams) => createWidget(context,weight,color,options),getWidget: (id: string,options?: SAPWidgetsGetWidgetOptionalParams) => getWidget(context,id,options),queryWidgetsPages: (page: number,pageSize: number,options?: SAPWidgetsQueryWidgetsPagesOptionalParams) => queryWidgetsPages(context,page,pageSize,options),listWidgetsPages: (page: number,pageSize: number,options?: SAPWidgetsListWidgetsPagesOptionalParams) => listWidgetsPages(context,page,pageSize,options),sapListWidgets: (requiredHeader: string,bytesHeader: Uint8Array,value: Uint8Array,csvArrayHeader: (Uint8Array)[],utcDateHeader: Date,options?: SAPWidgetsSAPListWidgetsOptionalParams) => sapListWidgets(context,requiredHeader,bytesHeader,value,csvArrayHeader,utcDateHeader,options)
          }
}
